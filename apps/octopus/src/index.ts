  ...
  async fetch(request, env) {
    try {
      ...
      if (url.pathname === '/onramp') {
        ...
        if (!address)
          return new Response('Address is required', { headers, status: 400 })

        const region = url.searchParams.get('region') ?? 'us'
        const destination_currency =
          url.searchParams.get('destination_currency') ?? 'usdc'
        const destination_network =
          url.searchParams.get('destination_network') ??
          (region === 'eu' ? 'ethereum' : 'base')
        const source_currency =
          url.searchParams.get('source_currency') ??
          (region === 'eu' ? 'eur' : 'usd')
        const source_amount = url.searchParams.get('source_amount') ?? '50'
        ...
        if (region === 'eu') {
          body.append('wallet_addresses[ethereum]', address)
          body.append('destination_currencies[]', 'usdc')
          body.append('destination_currencies[]', 'eth')
          body.append('destination_networks[]', 'ethereum')
        } else {
