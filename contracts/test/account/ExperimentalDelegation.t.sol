...
contract ExperimentalDelegationTest is Test {
    ...
    function test_initialize() public {
        ...
        ExperimentalDelegation.Key[]
            memory keys = new ExperimentalDelegation.Key[](1);
        keys[0] = ExperimentalDelegation.Key(
            0,
            ExperimentalDelegation.KeyType.P256,
            ECDSA.PublicKey(x, y)
        );
        ...
        (
            uint256 expiry,
            ExperimentalDelegation.KeyType keyType,
    ...
    function test_authorize_withAuthorizedKey() public {
        ...
        ExperimentalDelegation.Key[]
            memory nextKeys = new ExperimentalDelegation.Key[](1);
        nextKeys[0] = ExperimentalDelegation.Key(
            69420,
            ExperimentalDelegation.KeyType.P256,
            ECDSA.PublicKey(x, y)
        );
        ...
        ExperimentalDelegation.WrappedSignature
            memory wrappedSignature = ExperimentalDelegation.WrappedSignature(
                0,
    ...
    function test_execute_revertRevoke() public {
        ...
        vm.expectRevert(
            ExperimentalDelegation.KeyExpiredOrUnauthorized.selector
        );
        delegation.execute(calls, abi.encode(wrappedSignature));
    }
    ...
    function test_isValidSignature_forOwner() public {
        ...
        vm.etch(
            alice.addr,
            bytes.concat(hex"ef0100", abi.encodePacked(delegation))
        );
        ExperimentalDelegation delegation = ExperimentalDelegation(
            payable(alice.addr)
        );
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(delegation.nonce(), keccak256("0xdeadbeef"))
        );
        ...
        assertEq(
            delegation.isValidSignature(hash, signature),
